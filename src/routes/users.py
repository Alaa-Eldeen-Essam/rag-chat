...
from controllers import UserController, NLPController, ProjectController
from models.UserModel import UserModel
from models import ResponseSignal
from models.db_schemes import User, ChatHistory, ChatConversation, SummaryRecord, Asset, Project
...
async def _delete_asset_tree(
    ...
    *,
    ...
    if not asset_record or not exists_or_forbidden:
        return
    ...
    if chunk_ids:
        ...
        if vectordb_client:
            ...
            await vectordb_client.delete_records(collection_name, chunk_ids)
    ...
    try:
        project_path = project_controller.get_project_path(
            project_id=asset_record.asset_project_id
        )
        ...
        logger.error(
            "Failed to delete file for asset %s: %s",
...
async def delete_user(
    ...
    # Ensure the user exists and capture their assets before we mutate anything.
    async with request.app.db_client() as session:
        result = await session.execute(select(User).where(User.id == user_id))
        user_record = result.scalar_one_or_none()
        if user_record is None:
        ...
        asset_result = await session.execute(
            select(Asset.asset_id).where(Asset.asset_user_id == user_id)
        )
        asset_ids = asset_result.scalars().all()

    for asset_id in asset_ids:
        await _delete_asset_tree(
            request,
            asset_id,
            current_user,
    ...
    async with request.app.db_client() as session:
        async with session.begin():
            ...
            await session.execute(
                delete(SummaryRecord).where(SummaryRecord.user_id == user_id)
            )
            await session.execute(
                delete(Project).where(Project.project_user_id == user_id)
            )
            await session.execute(
